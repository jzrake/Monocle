...
// ============================================================================
TabulatedFunction::TabulatedFunction (const std::vector<double>& x,
...
TabulatedFunction::TabulatedFunction (double x0,
                                      double x1,
                                      int numberOfBins,
                                      BinSpacingMode spacingMode)
: spacingMode (spacingMode)
{
    for (int n = 0; n < numberOfBins + 1; ++n)
    {
        switch (spacingMode)
        {
            case BinSpacingMode::useEqualBinWidthsLinear:
            {
                xdata.push_back (x0 + (x1 - x0) * n / numberOfBins);
                break;
            }
            case BinSpacingMode::useEqualBinWidthsLogarithmic:
            {
                                                              ...
                                                              bool normalize)
{
    ...
    auto findNextBinLocation = [&] (double a)
    {
        switch (spacingMode)
        {
            case BinSpacingMode::useArbitraryBinSpacing:
            {
            ...
            case BinSpacingMode::useEqualBinMasses:
            {
    ...
    bool density, bool normalize, bool shift)
{
    if (samples.size() == 0)
    {
        return TabulatedFunction (std::vector<double>(), std::vector<double>(), BinSpacingMode::useArbitraryBinSpacing);
    }
    ...
    switch (spacingMode)
    {
        ...
        case BinSpacingMode::useEqualBinMasses:
        {
            throw std::runtime_error ("[TabulatedFunction::makeHistogram] bin spacing must be linear or logarithmic");
        }
        case BinSpacingMode::useEqualBinWidthsLinear:
        {
            for (int n = 0; n < numberOfBins + 1; ++n)
    ...
    if (shift)
    {
        ...
        // spacing here.
        return TabulatedFunction (xdata, ydata, BinSpacingMode::useArbitraryBinSpacing);
    }
    else
    {
...
size_t TabulatedFunction::size() const
{
    return xdata.size();
}
